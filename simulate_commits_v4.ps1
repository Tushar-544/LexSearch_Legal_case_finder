# ═══════════════════════════════════════════════════════════════════════
#  simulate_commits_v4.ps1
#  Creates an INCREMENTAL Git history for CODE ONLY.
#  
#  **NEW IN V4:** Solves the "0 files changed" issue on GitHub.
#  If a file appears in 4 different commits, it will write the first 25%
#  of the code in commit 1, 50% in commit 2, 75% in commit 3, and exactly
#  100% of the original final code in commit 4. Every commit will show 
#  real green file additions on GitHub.
# ═══════════════════════════════════════════════════════════════════════
...
Write-Host "  LexSearch - Advanced Incremental Git History" -ForegroundColor Cyan
Write-Host "  Prevents '0 file changes' via Line Revelation" -ForegroundColor Cyan
Write-Host "===================================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "[4/5] Building commits with incremental lines..." -ForegroundColor Yellow
...
foreach ($c in $commits) {
    ...
    foreach ($f in $c.Files) {
        if (-not $fileCur.ContainsKey($f)) { $fileCur[$f] = 0 }
        $fileCur[$f]++
        ...
        if (Test-Path $srcPath) {
            ...
            if ((Get-Item $srcPath).PSIsContainer) {
                ...
                # It's a file. Let's do partial revelation!
                $totalMentions = $fileFreq[$f]
                $currentMention = $fileCur[$f]
                
                if ($currentMention -eq $totalMentions) {
                    # Last time this file is committed: Copy raw file to guarantee perfection
                    Copy-Item -Path $srcPath -Destination $dstPath -Force
                } else {
                    # Partial revelation:
                    $rawContent = Get-Content -Path $srcPath -Encoding UTF8
                    if ($rawContent -is [string]) { $rawContent = @($rawContent) }
                    $totalLines = $rawContent.Count
                    
                    if ($totalLines -gt 0) {
                        $fraction = $currentMention / $totalMentions
                        $linesToWrite = [math]::Max(1, [math]::Round($fraction * $totalLines))
                        $partialContent = $rawContent | Select-Object -First $linesToWrite
                        Set-Content -Path $dstPath -Value $partialContent -Encoding UTF8
                    } else {
                        # Empty file fallback
                        Copy-Item -Path $srcPath -Destination $dstPath -Force
                    }
    ...
    # If nothing explicitly staged (e.g. all files bypassed), stage anything changed
...
Write-Host "  Complete! Repository history successfully rebuilt." -ForegroundColor White
Write-Host "===================================================" -ForegroundColor Green
